...
class Regressor(nn.Module):
    def __init__(self, opt_params, in_chans=2048, in_chans_pose=512):
        ...
        self.shape_out = make_linear_layers([in_chans, mano.shape_param_dim], relu_final=False)
        self.shape_out_seg = make_linear_layers([in_chans, mano.shape_param_dim * 2], relu_final=False)
        ...
        self.cam_out = make_linear_layers([in_chans, 3], relu_final=False)
        self.cam_out_seg = make_linear_layers([in_chans, 3 * 2], relu_final=False)

    def forward(self, feat_blur, feat_blur_seg, feat_joint_e1, feat_joint_md, feat_joint_e2, joint_img_e1, joint_img_md, joint_img_e2):
        # mano shape parameter regression
        mano_shape_md = self.shape_out(feat_blur.mean((2,3)))
        shape_param_seg = self.shape_out(feat_blur_seg.mean((2,3)))
        mano_shape_e1, mano_shape_e2 = torch.split(shape_param_seg, mano.shape_param_dim, dim=1)
        ...
        cam_param_md = self.cam_out(feat_blur.mean((2,3)))
        cam_param_seg = self.cam_out((feat_blur_seg.mean((2,3))))
        cam_param_e1, cam_param_e2 = torch.split(cam_param_seg, 3, dim=1)
